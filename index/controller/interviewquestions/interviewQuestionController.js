...
const InterviewQuestionController = {
  ...
  createInterviewQuestion: async (req, res) => {
    try {
      ...
      const interviewquestion = await InterviewQuestionModel.createInterviewQuestion(req.body);
      ...
      if(imgPath){
        ...
        const insertImage = await InterviewQuestionModel.insertInterviewImage(interviewQuestionId, imgPath, status);
      }  
  ...
  getInterviewQuestions: async (req, res) => {
    try{
      const interviewquestion = await InterviewQuestionModel.getInterviewQuestions();
  ...
  getInterviewQuestionById: async (req, res) => {
    try {
      ...
      const interviewquestion = await InterviewQuestionModel.getInterviewQuestionById(id);
      if(interviewquestion.length == 0){
        return res.status(404).json({
          response_code: 404,
          response_message: "Success",
  ...
  getInterviewQuestionsByPagination: async (req, res) => {
    try{
      ...
      const interviewquestion = await InterviewQuestionModel.getInterviewQuestionsByPagination(maincatid, subcatid, offset, limit);
  ...
  updateInterviewQuestion: async (req, res) => {
    try{
      var id = parseInt(req.params.id);
      const interviewquestion = await InterviewQuestionModel.updateInterviewQuestion(id, req.body);
      if(interviewquestion.length === 0){
        return res.status(404).json({ 
          response_code: 404,
          response_message : "Success",
  ...
  searchInterviewQuestion: async (req, res) => {
    try{
      const interviewquestion = await InterviewQuestionModel.searchInterviewQuestion(req.query, res);
  ...
  deleteInterviewQuestion: async (req, res) => {
    try{
      ...
      const interviewquestion = await InterviewQuestionModel.deleteInterviewQuestion(req.params, res);
