...
const ArticleCategoriesService = {
  ...
  getAllCategories: async () => {
    try {
      return await Pool.query(
        `select id,title,image_url,parent_category from article_categories where status = 1`
      );
    } catch (error) {
      console.error(error.message);
    }
  ...
  searchCategoriesById: async (id) => {
    try {
      const status = 1;
      return await Pool.query(
        `SELECT id, title, image_url, parent_category, status FROM article_categories WHERE id = $1 AND status = $2`,
        [id, status]
      );
    } catch (error) {
      console.error(error.message);
